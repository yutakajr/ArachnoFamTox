...
def run_hmmscan(fasta, args):
    ...
    subprocess.run(
        [
            ...
            str(args.eHMM),
            "--domtblout",
            Path(args.out, "out.hmmer.domtab"),
            path,  # here
            fasta,
        ],
...
def run_rpsblast(fasta, args):
    ...
    subprocess.run(
        [
            ...
            fasta,
            "-out",
            Path(args.out, "out.pssm"),
            "-evalue",
            str(args.ePSSM),
            "-outfmt",
            "6 qseqid sseqid evalue bitscore ppos pident qlen qstart qend",
            "-num_threads",
            str(args.cpu),
        ]
    )
...
def run_blastp(fasta, args):
    ...
    subprocess.run(
        [
            ...
            fasta,
            "-out",
            Path(args.out, "out.blastp.toxprot"),
            "-evalue",
            str(args.eBLASTP),
            "-outfmt",
            "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovs ppos salltitles",
            "-num_threads",
            str(args.cpu),
        ]
    )
                ...
                for i in range(0, num_hits):
                    if (hsps[i].evalue <= 0.1) or (hsps[i].evalue == 0):
                        output.write(
                            ...
                            + str(record.seq_len)
                            + "\t"
                            + str(hits[i].description)
                            + "\t"
                            + str(hsps[i].env_start)
                            + "\t"
                            + str(hsps[i].env_end)
                            + "\n"
                        )
