...
from .define_final_results import (
    define_final,
    gen_final_class_and_evalue,
    gen_score,
    filter_results,
    define_general_class,
    define_target_final,
)
from .util import (
    db_dir,
...
parser = argparse.ArgumentParser(
    prog="ArachnoFamTox",
    description="Prediction and Classification of Toxins and Venom Proteins for Arachnid species",
)
...
parser.add_argument(
    ...
    metavar="<evalue>",
    type=float,
    help="e-value for HMMSCAN. Default=1e-1",
    default=1e-1,
)
...
parser.add_argument(
    ...
    metavar="<evalue>",
    type=float,
    help="e-value for PSSM. Default=1e-5",
    default=1e-5,
)
...
parser.add_argument(
    ...
    metavar="<evalue>",
    type=float,
    help="e-value for BLASTp. Default=1e-5",
    default=1e-5,
)
...
parser.add_argument(
    ...
    metavar="<float>",
    type=float,
    help="Filter BLASTp output for qcovs >= <float>. Default=Off",
    default=0,
)
...
parser.add_argument(
    ...
    metavar="<float>",
    type=float,
    help="Filter BLASTp output for ppos >= <float>. Default=Off",
    default=0,
)
...
parser.add_argument(
    ...
    metavar="<float>",
    type=float,
    help="Filter BLASTp output for pident >= <float>. Default=Off",
    default=0,
)
...
parser.add_argument(
    ...
    metavar="<float>",
    type=float,
    help="Filter BLASTp output for evalue <= <evalue>. Default=1e-10",
    default=1e-10,
)
...
parser.add_argument(
    ...
    default=1,
    type=int,
    help="Specify number of threads. Default=1",
)
...
parser.add_argument(
    "--force", action="store_true", help="Force re-use output directory. Default=Off."
)

parser.add_argument(
    "--tempfiles", action="store_true", help="Maintain temporary files. Default=Off."
)

parser.add_argument(
    "--signalp", action="store_true", help="Perform Signalp analysis. Default=Off."
)
...
def merge_and_sort(tsv1, tsv2):
    ...
    file1 = pd.read_csv(
        tsv1, sep="\t", names=["0", "1", "2", "3", "4", "5", "6", "7", "8"]
    )  # Header
    file2 = pd.read_csv(
        tsv2, sep="\t", names=["0", "1", "2", "3", "4", "5", "6", "7", "8"]
    )
    concatfiles = pd.concat([file1, file2])
    sorted_concatfiles = concatfiles.sort_values(concatfiles.columns[0], ascending=True)
    sorted_concatfiles.to_csv(
        Path(args.out, "merged_outputs.tsv"), sep="\t", index=False, header=None
    )
        ...
        for line in t:
            ...
            if (query != prev_query) or (line == last_line):
                ...
                if bool(famHMM) == True:  # if HMM results exist, get final HMM result
                    result_hmm, result_hmm_final, evalue_hmm_final, target_hmm = (
                        gen_final_class_and_evalue(famHMM, "hmm")
                    )
                else:
                ...
                if (
                    bool(famPSSM) == True
                ):  # if PSSM results exist, get final PSSM result
                    result_pssm, result_pssm_final, bitscore_pssm_final, target_pssm = (
                        gen_final_class_and_evalue(famPSSM, "pssm")
                    )
                ...
                result_final, score = gen_score(
                    result_hmm_final, result_pssm_final
                )  # generate final merged result and score
                write = filter_results(result_final, score)  # filter results to write
                general_class = define_general_class(
                    result_final
                )  # define general class for final family
                target_final = define_target_final(
                    target_hmm, target_pssm
                )  # define final target
                ...
                if write == True:
                    ...
                    output.write(
                        ...
                        + str(general_class)
                        + "\t"
                        + normalize_family_name(str(result_final))
                        + "\t"
                        + str(target_final)
                        + "\t"
                        + str(score)
                        + "\t"
                        + normalize_family_name(str(result_pssm_final))
                        + "\t"
                        + normalize_family_name(str(result_hmm_final))
                        + "\t"
                        + str(evalue_pssm_final)
                        + "\t"
                        + str(evalue_hmm_final)
                        + "\t"
                        + str(bitscore_pssm_final)
                        + "\n"
                    )
                famHMM = {}
                famPSSM = {}
            ...
            if ("|TX" in hit) or (
                "|VP" in hit
            ):  # Save PSSM evalue and bitscore in dictionary
                pssm = hit.split("|")[0]
                ...
                if fam in famHMM:
                    famHMM[fam] *= float(
                        evalue_hmm
                    )  # multiply evalues if more than one for family
                else:
...
def gen_toxprot_output(toxprot, pssmhmm):
    ...
    with open(toxprot, "r") as tp:
        ...
        output.write(
            "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tqcovs\tppos\n"
        )
        for line in tp:
            header = line.split("\t")[0].strip()
            ...
            if header != prev_header:
                if header not in found_headers:
                    ...
                    prev_header = header
                    if (
                        (pident >= args.pidentfilter)
                        and (qcovs >= args.qcovsfilter)
                        and (ppos >= args.pposfilter)
                        and (evalue_tp <= args.evaluefilter)
                    ):
...
def insert_signalp_results(signalp, args):
    ...
    signalp_df = pd.DataFrame(
        signalp_lines,
        columns=["ID", "SP_Prediction", "SP_(Sec/SPI)", "SP_OTHER", "SP_CS Position"],
    )
    results_df = pd.read_csv(results_path, sep="\t", index_col=None, header=0)
    merged_df = pd.merge(
        results_df, signalp_df, left_on="Query", right_on="ID", how="left"
    )
...
def run():
    ...
    line_prepender(
        hmmscan,
        "record\thits\te-value\tbitscore\taccuracy\tlength\tdescription\tstart\tend\n",
    )
    line_prepender(
        rps,
        "qseqid\tsseqid\tpident\tlength\tmismatch\tgapopen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tqcovs\tppos\n",
...
def normalize_family_name(name):
    family_dict = {
        ...
        "CAP": "CAP",
        "Cystatin": "Cystatin",
        "Cytolytic Peptides": "Cytolytic Peptide",
        "Lycotoxin": "Lycotoxin",
        "Defensin": "Defensin",
